...
def get_last_scores():
    try:
        ...
        with open(file_path, "r") as f:
            lines = f.readlines()
        return lines[-3:][::-1]  # 3 plus récents, ordre chrono inversé
    except:
        return []
...
def display_intro():
    ...
    while len(pseudo) < 3:
        ...
        pseudo_display = " ".join(pseudo.ljust(3, "_"))
        pseudo_text = small_font.render(pseudo_display, True, white)
...
def menu():
    ...
    if not gameover:
        ...
        save_score(score, pseudo)  # Save score when game is over
...
def game_loop(small_font = pygame.font.SysFont('times new roman', 16), message=None):
    global snake_position, snake_body, fruit_position, fruit_spawn, direction, change_to, score, snake_speed, gameover, pseudo, message_time

    pygame.mixer.music.play(-1)
    volume = 0.5
    volume_surface = small_font.render(f"Volume : {volume: .1f}", True, white)
    volume_rect = volume_surface.get_rect(center=(window_x / 2, window_y / 4)) # Corriger position texte pour volume
        ...
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_w:
                    volume = max(0.0, volume - 0.1)  # Ne pas descendre sous 0
                    pygame.mixer.music.set_volume(volume)
                elif event.key == pygame.K_x:
                    volume = min(1.0, volume + 0.1)  # Ne pas dépasser 1.0
                    pygame.mixer.music.set_volume(volume)
            if event.type == pygame.QUIT:
                pygame.mixer.music.stop()
                pygame.quit()
