...
transparent_img = load_image('menu.png')
...
def display_intro():
    ...
    while len(pseudo) < 3:
        ...
        for i, line in enumerate([pygame.font.SysFont('times new roman', 24).render(score.strip(), True, green) for score in recent_scores]):
            rect = line.get_rect(midright=(window_x / 3.8, window_y / 3 - 120 + i * 25))
            game_window.blit(line, rect)

        for i, line in enumerate([pygame.font.SysFont('times new roman', 24).render(f"{p} - {s}", True, green) for p, s in top_scores]):
            rect = line.get_rect(midleft=(3 * window_x / 4, window_y / 3 - 120 + i * 25))
            game_window.blit(line, rect)

        display_text = " ".join(pseudo.ljust(3, "_"))
        text = small_font.render(display_text, True, green)
        rect = text.get_rect(center=(window_x / 2, window_y / 2.3))
